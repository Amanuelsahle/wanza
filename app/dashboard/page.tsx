...
export default function AdminDashboard() {
  const { products, showToast, setAddProductOpen, refreshProducts } =
    useWanza();
  ...
  // Build last-7-days chart data (day-to-day variation)
  ...
  const baselineChartData = [] as { name: string; revenue: number }[];
  for (let i = 6; i >= 0; i--) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    const key = d.toISOString().slice(0, 10);
    baselineChartData.push({
          ...
          <div>
            ...
            <h3 className="text-2xl font-extrabold text-slate-900 mt-2">
              ...
              {adminOrders.length > 0
                ? (totalRevenue / adminOrders.length).toFixed(2)
                : "0.00"}
            </h3>
            <p className="text-xs text-slate-500 font-semibold mt-1.5">
              Average ticket size
              ...
              adminOrders.map((order: any) => (
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-[9px] text-slate-400 mt-0.5">Paid</p>
                  </div>
                </div>
              ))
            )}
